...
(function () {
  ...
  // Atendimento (canal 1:1 com a equipe) — não confundir com o grupo acima,
  // que é comunidade. Número no formato do wa.me: 55 + DDD + número.
  var WHATSAPP_ATENDIMENTO = '5521959042832';
  ...
  function showSticky() {
    ...
    // O botão flutuante de atendimento vive no mesmo canto: sobe para não
    // ficar embaixo da barra. O timeout pega a altura já com a transição
    // aplicada (e depois de o conteúdo virar painel de referral).
    reposicionarFlutuante();
    setTimeout(reposicionarFlutuante, 450);
  }
  ...
  function initSticky() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      submitEmail(email, 'sticky_bar').finally(function () {
        ...
        // O painel de referral é mais alto que o formulário: remede.
        reposicionarFlutuante();
      });
    });
  }

  /* ================= WHATSAPP DE ATENDIMENTO (flutuante) =================
     Botão fixo no canto inferior direito, presente em todas as páginas que
     carregam este asset. Diferente dos outros widgets, NÃO respeita
     isSubscribed() nem cooldown: é canal de atendimento, não captura — quem
     já é lead continua precisando falar com a equipe.

     A mensagem pré-preenchida muda conforme a página para que a equipe saiba
     de onde a pessoa veio já na primeira linha da conversa, sem depender de
     analytics.
  */
  var WA_SVG = '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" focusable="false">'
  ...
  function contextoWhatsApp() {
    ...
    return { local: 'float-site', texto: 'Oi! Vim pelo site e queria tirar uma dúvida.' };
  }
  ...
  // chutar a altura dela — que muda no mobile, onde o layout quebra em duas
  // linhas, e de novo quando vira painel de referral — medimos na hora.
  function reposicionarFlutuante() {
    var flut = document.getElementById('fc-wa-float');
    if (!flut) return;
    var sticky = document.getElementById('fc-ec-sticky');
    if (sticky && sticky.classList.contains('fc-ec-show')) {
      flut.style.bottom = (sticky.offsetHeight + 16) + 'px';
    } else {
      flut.style.bottom = '';
    }
  }
  ...
  function initWhatsAppFloat() {
    if (document.getElementById('fc-wa-float')) return;

    var ctx = contextoWhatsApp();
    var a = document.createElement('a');
    a.id = 'fc-wa-float';
    a.className = 'fc-wa-float';
    a.href = urlWhatsApp(ctx.texto);
    a.target = '_blank';
    a.rel = 'noopener';
    a.setAttribute('aria-label', 'Falar com o atendimento no WhatsApp');
    a.setAttribute('data-fc-cta', 'whatsapp');
    a.setAttribute('data-fc-local', ctx.local);
    a.innerHTML = WA_SVG + '<span class="fc-wa-float-label">Fale com a gente</span>';

    a.addEventListener('click', function () { trackWhatsApp(ctx.local); });

    document.body.appendChild(a);

    // Entra depois do primeiro respiro da página, para não disputar atenção
    // com o conteúdo enquanto ele ainda está montando.
    setTimeout(function () { a.classList.add('fc-wa-show'); }, 900);

    window.addEventListener('resize', reposicionarFlutuante, { passive: true });
  }
